import './Components/shadow-preview.js'
...
import { BookDisplay, checkPreviewID } from "./Modules/bookUtility.js";
...
const handlerListItems = (event) => {
    ...
    const previewData = checkPreviewID(event);
    const clickPreview = document.querySelector('shadow-preview');
    clickPreview.open = true;
    clickPreview.title = previewData.title;
    clickPreview.subtitle = `${authors[previewData.author]} (${new Date(previewData.published).getFullYear()})`;
    clickPreview.description = previewData.description
    clickPreview.image = previewData.image
